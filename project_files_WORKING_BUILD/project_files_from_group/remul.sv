...
task wreg(input reg [4:0] ra,Rdata rd);
    if(ra != 0) Reg[ra]=rd;
$display("CHECKINGGGGGG %b %b %b",rd,ra,Reg[ra]);
endtask : wreg

function Rdata REG(reg [4:0] rix);
    if(rix==0) return 0;
$display("READINGGGGGG %b",rix);
    return Reg[rix];
endfunction : REG
...
task SRLI();
    Rdata w;
    w=REG(inst.R.rs1);
$display("WORKINGGGGGGGGGGGGGGGGGGGGG before %b",w);
    w=$unsigned(w)>>inst.R.rs2;
    wreg(inst.R.rd,w);
$display("WORKINGGGGGGGGGGGGGGGGGGGGG %b",w);
endtask : SRLI
